...
describe("LibGCDI-Profiles class-appropriate resource defaults", function()
	it("GetClassResourceDefaults enables only health and the class's own resources", function()
		local defaults = lib:GetClassResourceDefaults("ROGUE")
		assertTrue(defaults.health)
		assertTrue(defaults.energy)
		assertTrue(defaults.comboPoints)
		assertFalse(defaults.mana)
		assertFalse(defaults.rage)
		assertFalse(defaults.holyPower)
	end)

	it("GetClassResourceDefaults starts stagger disabled even for Monks", function()
		local defaults = lib:GetClassResourceDefaults("MONK")
		assertTrue(defaults.chi)
		assertTrue(defaults.energy)
		assertFalse(defaults.stagger)
	end)

	it("EnforceClassResources disables resources the class can't use and reports a change", function()
		local settings = { resourceSettings = { health = true, mana = true, rage = true, energy = false } }
		local changed = lib:EnforceClassResources(settings, "ROGUE")
		assertTrue(changed)
		assertTrue(settings.resourceSettings.health)
		assertFalse(settings.resourceSettings.mana)
		assertFalse(settings.resourceSettings.rage)
		assertFalse(settings.resourceSettings.energy)
	end)

	it("EnforceClassResources reports no change when everything already fits the class", function()
		local settings = { resourceSettings = { health = true, energy = true, comboPoints = false, mana = false } }
		local changed = lib:EnforceClassResources(settings, "ROGUE")
		assertFalse(changed)
		assertTrue(settings.resourceSettings.energy)
	end)

	it("EnforceClassResources is a no-op without resourceSettings", function()
		assertFalse(lib:EnforceClassResources({}, "ROGUE"))
		assertFalse(lib:EnforceClassResources(nil, "ROGUE"))
	end)
end)
