...
local libGCDI = libStub:NewLibrary(major, CONST_LIB_VERSION)
...
function libGCDI.load_position(frame, addonname, savevariable)
...
local function show_configpopup()
    for i, value in pairs(gpositions) do
        ...
        framelist[index] = nil;
        framelist[index] = setup_frame(framelist[index], "GCDI_frame" .. index, index, positions);
    end

    StaticPopup_Show("gcdiConfig")
end
...
do
    ...
    SlashCmdList['gcdi'] = show_configpopup
    SlashCmdList['gcdr'] = show_clearpopup
    SLASH_gcdi1 = '/gcdi'
    SLASH_gcdr1 = '/gcdr'
end
