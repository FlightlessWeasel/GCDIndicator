...
permanent detection mechanism. See `docs/dispel-indicator.md` for
...
> **Removed (2026-08-18):** this A/B toggle (`useNativeStackBinding`) was
> pulled out entirely — confirmed not working on the 12.1 test client
> (`SetApplicationBar` never painted a fill), parked, then deleted along
> with its `GCDIndicator.xml` template, Settings checkbox, and slash
> command. The `GCDIDispelOverlayTemplate` in `GCDIndicator.xml` (below,
